...
impl CStore {
    ...
    fn inject_panic_runtime(&mut self, tcx: TyCtxt<'_>, krate: &ast::Crate) {
        // If we need a panic runtime, we try to find an existing one here.
        if !attr::contains_name(&krate.attrs, sym::needs_panic_runtime) {
            return;
        }

        // Here we just load the panic runtimes.
        //
        ...
        // the activition of only one runtime.
        let mut resolve_panic_runtime = |name: Symbol| {
