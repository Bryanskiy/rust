...
use rustc_hir::{AmbigArg, HirId, LangItem};
    ...
    'tcx: 'a,
{
    ...
    pub(crate) fn add_implicit_trait(
        ...
        if !self.check_for_implicit_trait(trait_id, hir_bounds, self_ty_where_predicates) {
            ...
            push_lang_item_trait(self.tcx(), bounds, self_ty, trait_, span);
        }
    }
...
pub(crate) fn push_lang_item_trait<'tcx>(
    tcx: TyCtxt<'tcx>,
    bounds: &mut Vec<(ty::Clause<'tcx>, Span)>,
    ty: Ty<'tcx>,
    lang_item: LangItem,
    ...
    if lang_item == LangItem::Sized {
        ...
        bounds.push((trait_ref.upcast(tcx), span));
    }
}
