...
struct FindMin<'a, 'tcx, VL: VisibilityLike, const SHALLOW: bool> {
    ...
    min: VL,
    type_to_impls: &'a mut FxHashMap<LocalDefId, FxHashSet<LocalDefId>>,
    ...
    for FindMin<'a, 'tcx, VL, SHALLOW>
{
    ...
    fn visit_def_id(&mut self, def_id: DefId, _kind: &str, _descr: &dyn fmt::Display) {
        if let Some(def_id) = def_id.as_local() {
            if Self::SHALLOW {
                self.type_to_impls.entry(def_id).or_default().insert(self.impl_def_id);
            }
...
trait VisibilityLike: Sized {
    ...
    fn of_impl<const SHALLOW: bool>(
        ...
        type_to_impls: &mut FxHashMap<LocalDefId, FxHashSet<LocalDefId>>,
    ) -> Self {
...
impl<'tcx> EmbargoVisitor<'tcx> {
    ...
    fn check_def_id(&mut self, def_id: LocalDefId) {
        ...
        match def_kind {
            ...
            | DefKind::Ctor(..) => {
                bug!("{:?} should be checked while checking parent", def_kind)
            }
        }
    }
}
...
impl ReachEverythingInTheInterfaceVisitor<'_, '_> {
    ...
    fn try_append_item_in_queue(&mut self, def_id: LocalDefId) {
        ...
        let check_impls = |ev: &mut EmbargoVisitor<'_>, def_id| {
            if let Some(impls) = ev.type_to_impls.get(&def_id) {
                // The order in which items are traversed is irrelevant.
                #[allow(rustc::potential_query_instability)]
                ev.queue.extend(impls.iter());
            }
        };

        match self.ev.tcx.def_kind(def_id) {
            DefKind::Enum
            | DefKind::Union
            | DefKind::Struct
            | DefKind::ForeignTy
            ...
            | DefKind::Fn => {
                ...
                check_impls(&mut self.ev, def_id)
            }
            DefKind::AssocConst { .. } | DefKind::AssocTy | DefKind::AssocFn => {
                // FIXME: `EmbargoVisitor` can't traverse these defs(see `check_def_id`).
            ...
            DefKind::Ctor(ctor_of, _) => {
                ...
                let update_id = match ctor_of {
                    ...
                    CtorOf::Variant => {
                        let variant = self.ev.tcx.local_parent(def_id);
                        self.ev.tcx.local_parent(variant)
                    }
                };
